...
# modified OPENRAM path:
export MODIFIED_OPENRAM = /home/zexuany2/Desktop/411_CA/OpenRAM/
export CONDA_HOME=$(MODIFIED_OPENRAM)/miniconda
export OPENRAM_HOME=$(MODIFIED_OPENRAM)/compiler
export OPENRAM_TECH=$(MODIFIED_OPENRAM)/technology
...
all:
	...
	python3 sram.py
	sync
	$(MAKE) --jobs=$(shell echo $(shell nproc)-2 | bc) $(notdir $(basename $(wildcard $(PWD)/output/*/*.py)))
...
%:
	...
	rm -f lc*.log lc*.txt
	# newly added: copy both arrays into hdl
	cp $(PWD)/output/$(basename $(notdir $@))/$(basename $(notdir $@)).v $(PWD)/hdl
